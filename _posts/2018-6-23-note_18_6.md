...
这个算法依然是基于sweep line的，也就是拿一条平行于$x$轴的直线从上往下扫过待分割的多边形$\mathcal P$，$\mathcal P$的顶点集合就是所有的event point。现假设$v_1, \dots, v_n$为$\mathcal P$所有顶点的一个逆时针排列，$e_1 = \overline {v_1v_2}$，$e_2 = \overline {v_2v_3}$，…，$e_n = \overline {v_nv_1}$，我们的目的是用适当的diagonals干掉所有的split vertex和merge vertex。
...
**Top & Bottom:** 每个梯形$\Delta$一定有两个non-vertical sides，上面那个所对应的原$\mathcal S \cup R$中的边记作$\mathrm{top}(\Delta)$，下面那个对应的原$\mathcal S \cup R$中的边记作$\mathrm{bottom}(\Delta)$。

**Left & Right:** 除了整个$\mathcal T(\mathcal S)$中最左侧和最右侧的两个矩形外，每个梯形$\Delta$左侧如果有条垂直边，那么记引出该垂直边的那个原$\mathcal S$中的顶点为$\mathrm{leftp}(\Delta)$，否则记$\mathrm{top}(\Delta)$和$\mathrm{bottom}(\Delta)$的唯一交点（位于$\Delta$的左侧）为$\mathrm{leftp}(\Delta)$。类似地，可以定义$\mathrm{rightp}(\Delta)$。

总而言之，一个梯形$\Delta$由它的上下边界线$\mathrm{top}(\Delta)$、$\mathrm{bottom}(\Delta)$和左右定界点$\mathrm{leftp}(\Delta)$、$\mathrm{rightp}(\Delta)$唯一地确定。
