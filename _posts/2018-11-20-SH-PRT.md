...
是归一化系数。根据$l$的不同，可以把SH分成很多层，层数越高所代表的分量的频率也就越高。比如$l = 0$对应1个SH，就只是个常量函数；$l = 1$对应3个SH，有了一些粗粒度的变化；$l = 2$时对应5个SH，描述了高频率的分量，等等：

![PICTURE]({{site.url}}/postpics/Atrc/3-order-sh-bands.png)

一般我们用“$L$阶SH”来代指$l = 0, \ldots, L-1$所对应的函数族。
...
现取$N_0 = [1, 0, 0]^T, N_1 = [0, 1, 0]^T, N_2 = [0, 0, 1]^T$，则$A$及其逆为：

$$
A = \left[P(N_0), P(N_1), P(N_2)\right] = \frac 1 2\sqrt{\frac{3}{\pi}}\left[\begin{matrix}
    0 & 1 & 0 \\
    0 & 0 & 1 \\
    1 & 0 & 0
\end{matrix}\right]
~~~~
A^{-1} = 2\sqrt{\frac{\pi}{3}}\left[\begin{matrix}
    0 & 0 & 1 \\
    1 & 0 & 0 \\
    0 & 1 & 0
\end{matrix}\right]
...
我懒得算了，直接抄这个算法出处给的数据吧。设$k = 1/\sqrt{2}$，取：
