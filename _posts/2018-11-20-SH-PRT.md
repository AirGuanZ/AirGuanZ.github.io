...
$$
\begin{aligned}
    \hat t_k &= \frac 1 N \sum_{i=1}^N \frac{f_s(\Phi_i \to x \to \Theta)V(x \to \Phi_i)\cos\langle N_x, \Phi_i\rangle y^{(k)}(\theta_{\Phi_i}, \phi_{\Phi_i})}{p_S(\Phi_i)} \\
    \hat l_k &= \frac 1 N \sum_{i=1}^N \frac{L_E(\Phi_i)y^{(k)}(\theta_{\Phi_i}, \phi_{\Phi_i})}{p_S(\Phi_i)}
\end{aligned}
$$
