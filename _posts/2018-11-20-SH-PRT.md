...
是归一化系数。根据$l$的不同，可以把SH分成很多层，层数越高所代表的分量的频率也就越高。比如$l = 0$对应1个SH，就只是个常量函数；$l = 1$对应3个SH，有了一些粗粒度的变化；$l = 2$对应5个SH，描述了高频率的分量，等等：

![PICTURE]({{site.url}}/postpics/Atrc/3-order-sh-bands.png)

一般我们用“L阶SH”来代指$l = 0, \ldots, L-1$所对应的SH。
...
我简单地实现了3阶SH的投影、系数旋转和重建过程，代码放在[这里](https://github.com/AirGuanZ/Atrc/tree/master/Source/Tools/SH)。随便画个模型看看：

![PICTURE]({{site.url}}/postpics/Atrc/2018_11_27_SHRotation.png)

环境光一开始在人像的右侧，上图中从左至右是将环境光绕垂直方向旋转了0度、90度、180度、270度后的结果，效果意外地很不错。
