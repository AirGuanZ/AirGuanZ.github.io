...
$$
说人话就是距离 = 速度 x 时间，且起始区域的“抵达时间”为0。如果$F$仅依赖于$p$，那么就得到了Eikonal Equation。
...
$$
7. 重复后面的步骤直到$\mathcal A$为空。
8. 令$p = \text{argmin}_{p \in \mathcal A}T(p)$，将$p$转入$\mathcal F$。
9. 对$N(p)$的每个点$n$，若$n \in \mathcal U$，则将其转入$\mathcal A$。
10. 对$N(p)$中的每个点$n$，若$n \notin \mathcal F$，则根据Eikonal Equation更新$T(n)$。
...
(T(n) - x)^2 + (T(n) - y)^2 = 1
$$
容易该方程一定有解，且两个解中只有更大的那一个比$x$和$y$都更大，因此，`else`分支中的代码为：
