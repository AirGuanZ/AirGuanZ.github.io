...
如果某个顶点$x_i(2 \le i \le k-2)$处的散射是specular，那么在这里没法做vertex merging，有几个$d$需要对应地做出修改：

$$
\begin{aligned}
\tau &= i + 1 \\
d_{\mathrm{PT}_{\tau}} &= \frac{p(x_{\tau-1}\leftarrow x_{\tau})}{p(x_{\tau-2}\to x_{\tau-1})}d_{\mathrm{PT}_{\tau-1}} \\
d_{\mathrm{VM}\rightarrow_\tau} &= \frac{p(x_{\tau-1}\leftarrow x_{\tau})}{p(x_{\tau-2}\to x_{\tau-1})}d_{\mathrm{VM}\rightarrow_{\tau-1}} \\
d_{\mathrm{VM}\leftarrow_\tau} &= \frac{p(y_{\tau-1}\leftarrow y_{\tau})}{p(y_{\tau-2}\to y_{\tau-1})}d_{\mathrm{VM}\leftarrow_{\tau-1}}
\end{aligned}
$$

另外，如果$x_{k-1}$是specular vertex，那么这条路径是没有光源采样策略的，$p_\mathrm{light}$对应的pdf为0。
