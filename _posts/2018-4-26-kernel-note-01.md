...
5. 要启用分页，需要先填充初始的页目录和页表（x86采用两级页表），而填充的内容决定了进入虚拟地址空间后的内存布局。对此，bootloader将物理内存的0~4MB占用，并给出了以下从虚拟地址到物理地址的转换：
   ...
   $\text{0x00000000} \Rightarrow \text{0x000fffff}$和实模式下的用处相同，为内核代码和显存区域
   
   $\text{0x00100000} \Rightarrow \text{0x001fffff}$为内核栈区域，即内核进程所使用的栈（挺奢侈的）
   
   $\text{0x00200000} \Rightarrow \text{0x002fffff}$为内核虚拟地址空间的页目录以及前255个页表
   
   $\text{0x00300000} \Rightarrow \text{0x003fffff}$为内核保留区域
